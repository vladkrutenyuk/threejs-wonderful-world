...
import { Vector3, Scene, Color, PerspectiveCamera, WebGLRenderer, Mesh, AxesHelper, Material, IcosahedronGeometry, MeshBasicMaterial, Texture, TextureLoader, CubeTextureLoader} from '/build/three.module.js'
...
const sphereMat: Material = new THREE.MeshBasicMaterial({ color: 0x000000, wireframe: true })
const sphere: Mesh = new Mesh(sphereGeometry, sphereMat)

const cubeGeometry: THREE.BoxGeometry = new THREE.BoxGeometry()
const cubeMaterial: Material = new THREE.MeshBasicMaterial({ color: 0x000000, wireframe: true })
const cube: Mesh = new Mesh(cubeGeometry, cubeMaterial)

const icoGeometry : IcosahedronGeometry = new IcosahedronGeometry
const icoMat : MeshBasicMaterial = new MeshBasicMaterial()
const ico : Mesh = new Mesh(icoGeometry, icoMat)
