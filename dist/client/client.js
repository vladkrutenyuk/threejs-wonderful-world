...
import { Scene, Color, PerspectiveCamera, WebGLRenderer, Mesh, MeshBasicMaterial, TextureLoader, Vector2, PlaneGeometry, RingGeometry, Raycaster, PointLight, CylinderGeometry } from '/build/three.module.js';
const scene = new Scene();
scene.background = new Color(0x101010);
const camera = new PerspectiveCamera(55, window.innerWidth / window.innerHeight, 0.1, 150);
...
const planeGeometry = new PlaneGeometry(planeData.width, planeData.height, planeData.widthSegments, planeData.heightSegments);
var planeMatData = {
...
function onDoubleClick(event) {
    ...
    raycaster.setFromCamera(mousePos, camera);
    var intersect = raycaster.intersectObject(planeMesh);
    const column = new Mesh(new CylinderGeometry(columnData.radiusTop, columnData.radiusBottom, columnData.height), new MeshBasicMaterial({ color: 0xff0000 }));
